    ...
    $pdf->Cell(20, 8, 'Carnet', 1, 0, 'C', true);
    $pdf->Cell(30, 8, 'Ubicacion', 1, 1, 'C', true);

    $pdf->SetFont('Arial', '', 10);
    $query = mysqli_query($conn, "SELECT * FROM usuarios ORDER BY nombre DESC");

    while ($row = mysqli_fetch_assoc($query)) {
        $pdf->Cell(30, 8, utf8_decode($row['nombre']), 1);
        $pdf->Cell(30, 8, utf8_decode($row['apellido']), 1);
        $pdf->Cell(50, 8, utf8_decode($row['correo']), 1);
        $pdf->Cell(30, 8, utf8_decode($row['telefono']), 1);
        $pdf->Cell(20, 8, utf8_decode($row['ci']), 1);
        $pdf->Cell(30, 8, utf8_decode($row['ubicacion']), 1);
        $pdf->Ln();
    }
